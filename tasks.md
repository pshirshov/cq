...
- [x] **fts-2** (commit pending) — Add `ftsSearch` to `LedgerStore`; wire `LedgerSearchIndex` into both adapters (init builds active+archived; onMutation rebuilds active [+archived on archive op]; invalidate rebuilds active+archived for BOTH known-ledger and registry-reload branches; createLedger indexes empty). Active rebuild guarded so it never unwinds the write. Dual-tests: ranked cross-ledger, single-ledger filter, boosts, fuzzy/prefix, status, includeArchived (archive→found only when true), local coherence.
