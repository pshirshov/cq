...
- [x] **fts-3** (commit pending) — Cross-process coherence test: two FsLedgerStore on one cwd, onMutation→peer.invalidate relay; B.ftsSearch finds A's item only after invalidate (proves index rebuilds on remote path).
