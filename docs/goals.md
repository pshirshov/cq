...
- updatedAt: 2026-06-02T22:38:31.843Z
- author: "opus-4.8[1m]"
- session: fe0aaf85-56b3-45ce-a7fc-718ab19c37e1
...
- description: |
    ...
    [Original D9/D10/D11/D12 scope omitted here for brevity — ALL RESOLVED: D9→T105, D10→T106, D11→T107, D12→T108+T109 (commits 8058527/c63d63d/e266ca4/9d3c068/8dfc415), decisions K17+K18, work milestone M28. See git history + the resolved defects D9–D12 for the full original text.]
    
    ## Follow-up (2026-06-02, #2) — universal /advance command + subagent parallelism bump
    (Item 1 of that request — archived-milestone titles — was first misdiagnosed as a stale server; see Follow-up #3 below for the CORRECTED diagnosis + filed defect D16.)
    
    2) UNIVERSAL /advance COMMAND (verbatim): 'create a universal command /advance which would combine plan:advance investigate:advance and implement:advance, the idea is simple: a) it checks if there is anything to investigate, if so it runs investigate:advance, then - checks if there is anything to plan and invokes plan:advance, after that it checks if there is anything what could be implemented - runs implement:advance, if there is nothing to do (ledgers drained or everything is blocked by questions) - it explains that.' Clarify: detection predicates (ledger queries) for anything-to-investigate/plan/implement; one-pass vs loop-until-quiescent; re-check ordering (investigate seeds goal → re-plan; implement files defects → re-investigate); composition with K12 stop predicates + subagents-cannot-spawn-subagents (a command may chain commands); 'nothing to do' report (drained vs blocked-on-questions). Wire into scripts/link-prompts.ts LINKS + .codex/prompts. (Clarifying questions Q55–Q60 filed.)
    
    3) SUBAGENT PARALLELISM (verbatim): 'increase subagent parallelism to 8 from current 4.' Raise the implement-flow concurrent-worker cap N=4→8 in llm/commands/implement/advance.md (Concurrency rule, line 36) + any restatement in implement/start.md. (Covered by Q60.)
    
    ## Follow-up (2026-06-02, #3) — ledger archive-and-reset command (+ corrected archived-titles diagnosis; 3 defects filed)
    GREENFIELD tooling, folded into G6 per explicit user direction. Verbatim request: 'we need a command to archive and reset ledgers. It could be something like ledger-reset or ledger-mcp --reset.' Clarify/plan: exact surface (standalone `ledger-reset` binary vs `ledger-mcp --reset` flag vs both); operational meaning of 'archive and reset' (archive all active milestones/items then re-init the canonical empty set? OR back-up-and-reinit reusing the G4/T94 backupAndReinit helper? destructive vs backup-first); scope (all ledgers vs selectable); safety/confirmation prompt; relationship to the existing FsLedgerStore backup-and-reinit (G4 / T94 / T95). Product code in @cq/ledger-mcp (+ @cq/ledger helper) + tests; repo gate bun run check.
    
    DEFECTS FILED THIS ROUND (NOT folded as goal scope — defect/scope separation; route via /investigate:start or a defect-seeded fix plan):
    - D16 (medium, CONFIRMED): archived milestone titles missing in NON-milestones ledger views (tasks/defects/etc.). Root cause: T109's backfillLegacyArchivePointers is gated `if (isMilestones)` (FsLedgerStore.ts:402), so only the milestones ledger's archivePointers are backfilled; tasks 19/20 + defects 3/4 keep empty titles, and ArchiveSubsections heads (label = pointer.title, T91/D8) render title-less. CONFIRMS the user's refutation of the 'stale server' theory. Means D12's fix (T109) was scoped too narrowly. ledgerRefs T109/D12/G6.
    - D17 (low): milestones-ledger archived ROW renders the archived badge inside the id column (App.tsx:1862) → wraps 'M13' + badge. Should not be in the id cell. ledgerRefs T108/D12/G6.
    - D18 (medium): batch answer-questions modal lacks per-suggestion 'pick' buttons (present only in the detail view; App.tsx:2438 vs the batch modal ~1392-1475). ledgerRefs T86/T88/G2.
- grounding: |
    D9/D10/D11/D12 scope RESOLVED (see resolved defects + git). Follow-up #2 (Q55–Q60): the only explicit concurrency cap is N=4 at llm/commands/implement/advance.md:36; a new /advance command wires into scripts/link-prompts.ts LINKS (lines 29-43) + .codex/prompts mirror; /advance would be the first no-namespace top-level command.
    Follow-up #3 (ledger-reset): relates to the G4/T94 backupAndReinit helper (timestamped docs/.backup/ + fresh canonical) in @cq/ledger FsLedgerStore — a reset command likely reuses/extends it; surface in @cq/ledger-mcp (a CLI flag like --reset, or a sibling bin). CONFIRMED defect D16: probe of FsLedgerStore.init() over a copy of live docs shows fetch('milestones').archivePointers 0/20 empty titles but fetch('tasks') 19/20 empty + fetch('defects') 3/4 empty — the isMilestones gate at FsLedgerStore.ts:402. D17 grounded at App.tsx:1862 (lw-archived-badge in archived row id cell). D18 grounded at App.tsx:2438 (detail pick buttons) vs BatchAnswerModal ~1392-1475 (no per-suggestion pick).
